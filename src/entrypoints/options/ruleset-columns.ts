    ...
    cell: ({ row }) => {
      return renderComponent(RulesetToggle, {
        ...
        onCheckedChange: async () => {
          ...
          if (!ruleset.active) {
            browser.userScripts.unregister({ ids: [row.original.id] })
            toast(ToastHTML, {
              componentProps: {
                message: `${row.original.name} - <b>Disabled</b>`,
              },
            })
          } else {
            ...
            toast(ToastHTML, {
              componentProps: {
                message: `${row.original.name} - <b>Enabled</b>`,
              },
            })
          }
        },
    ...
    cell: ({ row }) => {
      return renderSnippet(
        createRawSnippet(() => {
          return {
            render: () =>
              `<p>${new Date(row.original.updated).toLocaleString()}</p>`,
          }
        }),
        {},
      )
